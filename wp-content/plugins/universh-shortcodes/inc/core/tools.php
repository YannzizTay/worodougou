 ...
 */
function wt_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
...
function wt_hex2rgb( $colour, $delimiter = '-' ) {
	if ( $colour[0] == '#' ) {
 ...
 */
function wt_apply_formatting() {
	// Enable shortcodes in text widgets
	add_filter( 'widget_text', 'do_shortcode' );
	// Enable shortcodes in category descriptions
	add_filter( 'category_description', 'do_shortcode' );
	// Enable custom formatting
	if ( get_option( 'wt_option_custom-formatting' ) === 'on' ) {
		// Apply custom formatter function
		add_filter( 'the_content', 'wt_clean_shortcodes' );
	}
}

add_action( 'init', 'wt_apply_formatting' );
 ...
 */
function wt_clean_shortcodes( $content ) {
	$p = wt_cmpt();
	$array = array (
		'<p>[' => '[',
		']</p>' => ']',
 ...
 */
function wt_do_shortcode( $content, $pre ) {
 ...
 */
function wt_compatibility_mode_prefix() {
	return get_option( 'wt_option_prefix' );
}

/**
 * Shortcut for wt_compatibility_mode_prefix()
 */
function wt_cmpt() {
	return wt_compatibility_mode_prefix();
}
 ...
 * @return bool
 */
function wt_addon_active( $addons ) {
	// Prepare add-ons paths
	$paths = array(
		'maker' => 'Universh-maker/Universh-maker.php',
		'skins' => 'Universh-skins/Universh-skins.php',
		'extra' => 'Universh-extra/Universh-extra.php',
	);
...
function wt_skins_link() {
	if ( wt_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'universh' ), __( 'Open dropdown to choose one of new styles', 'universh' ) );
	else return sprintf( '<br><a href="http://gndev.info/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'universh' ) );
}
 ...
 */
function wt_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	global $wp_version;
 ...
 */
function wt_delete_resized_images( $post_id ) {
...
add_action( 'delete_attachment', 'wt_delete_resized_images' );

class Wt_Tools {
	function __construct() {
		add_action( 'wp_ajax_wt_example_preview', array( __CLASS__, 'example' ) );
		add_action( 'ts/update',                  array( __CLASS__, 'reset_examples' ) );
		add_action( 'ts/activation',              array( __CLASS__, 'reset_examples' ) );
	...
	public static function get_slides( $args ) {
		...
		if ( $args['gallery'] !== null || ( $args['source'] === 'none' && get_option( 'wt_option_galleries-432' ) ) ) return self::get_slides_432( $args );
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				elseif ( $args['link'] === 'custom' ) $slide['link'] = get_post_meta( $post->ID, 'wt_slide_link', true );
	...
	public static function get_slides_432( $args ) {
		...
		$galleries = get_option( 'wt_option_galleries-432' );
		// No galleries found
	...
	public static function example() {
		...
		self::access();
		// Check nonce
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'wt_examples_nonce' ) ) return;
		// Check incoming data
		if ( !isset( $_REQUEST['code'] ) || !isset( $_REQUEST['id'] ) ) return;
		// Check for cache
		$output = get_transient( 'ts/examples/render/' . sanitize_key( $_REQUEST['id'] ) );
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="wt-examples-error">' . __( 'Example code does not found, please check it later', 'universh' ) . '</p>' );
			// Clean-up the code
			$code = str_replace( array( "\t", '%wt_' ), array( '  ', wt_cmpt() ), $code );
			// Split code
			$chunks = explode( '-----', $code );
			// Show snippets
			do_action( 'ts/examples/preview/before' );
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="wt-examples-code"><span class="wt-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'universh' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
			do_action( 'ts/examples/preview/after' );
			$output = ob_get_contents();
			ob_end_clean();
			set_transient( 'ts/examples/render/' . sanitize_key( $_REQUEST['id'] ), $output );
			echo $output;
		}
		die();
	}
	...
	public static function reset_examples() {
		foreach ( (array) Wt_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'ts/examples/render/' . $item['id'] );
	}
	...
	public static function icons() {
		$icons = array();
		if ( is_callable( array( 'Wt_Data', 'icons' ) ) ) foreach ( (array) Wt_Data::icons() as $icon ) {
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['wt_slide_link'] = array(
			'label' => __( 'Slide link', 'universh' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'wt_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'Universh Shortcodes', 'universh' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'universh' ) )
		);
		return $form_fields;
	}

	public static function slide_link_save( $post, $attachment ) {
		if ( isset( $attachment['wt_slide_link'] ) )
			update_post_meta( $post['ID'], 'wt_slide_link', $attachment['wt_slide_link'] );
		return $post;
	}

	public static function error( $prefix = false, $message = false ) {
		if ( !$prefix && !$message ) return '';
		$return = array( '<div class="wt-error" style="padding:10px;border:1px solid #f03;color:#903;background:#fde">' );
...
/**
 * Shortcut for Wt_Tools::decode_shortcode()
 */
function wt_scattr( $value ) {
	return Wt_Tools::do_attr( $value );
}

/**
 * Shortcut for Wt_Tools::get_icon()
 */
function wt_get_icon( $args ) {
	return Wt_Tools::get_icon( $args );
}
